...
class Module2Adapter(ModuleInterface):
    """模块二适配器 - 实现标准模块接口"""
    ...
    def is_available(self) -> bool:
        ...
        try:
            ...
            # 检查Segmentations模块是否可用
            segmentations_module = slicer.modules.segmentations
            if not segmentations_module:
                return False
            
            # 检查Markups模块是否可用（用于标志点）
            markups_module = slicer.modules.markups
            if not markups_module:
                return False
            
            # 检查必要的MRML节点类型
            from slicer import vtkMRMLSegmentationNode, vtkMRMLMarkupsFiducialNode
    ...
    def create_widget(self, session: TAVRStudySession, parent=None):
        ...
        if self._logic is None:
            self._logic = Module2Logic()
        
        # 创建并返回组件
        self._widget = Module2Widget(session, logic=self._logic, parent=parent)
        return self._widget
    
    def get_dependencies(self) -> List[str]:
        """获取模块依赖"""
        return ["module1"]  # 模块二依赖模块一
